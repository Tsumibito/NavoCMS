...
import { PinnedMediaProcessor } from "./processor.js";
...
describe("pinned media processor", () => {
  it("emits deterministic AVIF, WebP, and JPEG variants with bounded transforms", async () => {
    ...
    const processor = new PinnedMediaProcessor();
  ...
  it("rejects unpinned sizes and invalid focal points before encoding", async () => {
    ...
    const processor = new PinnedMediaProcessor(); const preset = resolvePreset("thumbnail", "v1");
    await expect(processor.process({ bytes: source, mediaType: "image/png", preset, width: 999, format: "image/webp", crop: "center" })).rejects.toThrow("TRANSFORM");
    await expect(processor.process({ bytes: source, mediaType: "image/png", preset, width: 160, format: "image/webp", crop: "focal", focalPoint: { x: 2, y: 0 } })).rejects.toThrow("FOCAL");
    await expect(processor.process({ bytes: source, mediaType: "image/png", preset, width: 160, format: "image/webp", crop: "center", focalPoint: { x: 0.5, y: 0.5 } })).rejects.toThrow("FOCAL");
    await expect(processor.process({ bytes: source, mediaType: "image/png", preset: resolvePreset("responsive", "v1"), width: 320, format: "image/webp", crop: "focal", focalPoint: { x: 0.5, y: 0.5 } })).rejects.toThrow("CROP");
    await expect(processor.process({ bytes: source, mediaType: "image/png", preset, width: 160, format: "image/webp", crop: "unknown" as "center" })).rejects.toThrow("CROP");
    await expect(processor.process({ bytes: source, mediaType: "image/png", preset, width: 160, format: "image/webp", crop: "focal", focalPoint: null as unknown as { x: number; y: number } })).rejects.toThrow("FOCAL");
  });
  ...
  it("deep-freezes preset policy and strips orientation metadata", async () => {
    ...
    const result = await new PinnedMediaProcessor().process({
