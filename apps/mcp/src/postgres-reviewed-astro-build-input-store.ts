...
function storedRender(render: AstroRenderInput): StoredRender {
  return { tenantId: render.tenantId, siteId: render.siteId, locales: structuredClone(render.locales), anchors: structuredClone(render.anchors), deliveryLayout: structuredClone(render.deliveryLayout), expectedMediaDigest: render.expectedMediaDigest,
    design: { digest: render.design.digest, css: render.design.css, components: [...render.design.components.values()].map((item) => ({ id: item.id, module: item.module, source: item.source ?? null, exportName: item.exportName ?? null })), recipes: structuredClone(render.design.recipes), legacyComponentIds: [...render.design.legacyComponentIds] }, routes: structuredClone(render.routes) };
}
function readStoredRender(value: unknown): AstroRenderInput {
  ...
  for (const component of stored.design.components) {
    if (!component || typeof component !== "object" || Array.isArray(component) || !exactKeys(component, ["id", "module", "source", "exportName"]) || typeof component.id !== "string" || ids.has(component.id)) throw new McpEditingError("REVIEWED_ASTRO_BUILD_INPUT_CORRUPT", "Reviewed Astro render snapshot is invalid");
  ...
  return freeze({ tenantId: stored.tenantId, siteId: stored.siteId, locales: structuredClone(stored.locales), anchors: structuredClone(stored.anchors), deliveryLayout: structuredClone(stored.deliveryLayout), expectedMediaDigest: stored.expectedMediaDigest,
    design: { digest: stored.design.digest as `sha256:${string}`, css: stored.design.css, components: new Map(stored.design.components.map((component) => [component.id, freeze({ id: component.id, module: component.module, ...(component.source === null ? {} : { source: component.source }), ...(component.exportName === null ? {} : { exportName: component.exportName }) })])), recipes: structuredClone(stored.design.recipes), legacyComponentIds: [...stored.design.legacyComponentIds].sort() }, routes: structuredClone(stored.routes) });
}
