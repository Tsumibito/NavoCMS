...
const selection = Object.freeze({
  ...
  binding: Object.freeze({ schema: "io.navocms.r2-runtime-binding.v1" as const, tenantId, siteId, environment: "staging" as const, endpoint: "https://account.r2.cloudflarestorage.com", bucket: "navi-training", namespace: "navocms/v1/" as const, accessKeySecretRef: "secret:r2/access-key", secretKeySecretRef: "secret:r2/secret-key" }),
  ...
  secrets: Object.freeze({ assertAvailable: () => undefined, use: async <T>(reference: string, operation: (value: string) => Promise<T>) => operation(reference.includes("access") ? "access-key" : "secret-key") })
}) satisfies R2RuntimeSelectionResult;
...
describe("activated R2 storage runtime", () => {
  it("proves both exact child namespace markers without writes or deletes", async () => {
    ...
    const runtime = createR2StorageRuntime({ selection, tenantId, siteId, transport });
  ...
  it("fails closed on marker drift and scope mismatch", async () => {
    ...
    await expect(createR2StorageRuntime({ selection, tenantId, siteId, transport }).ready()).resolves.toBe(false);
    expect(() => createR2StorageRuntime({ selection, tenantId: "other", siteId, transport })).toThrow("SCOPE_MISMATCH");
  });
});
